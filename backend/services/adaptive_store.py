...
def get_firebase_uid(request):
    ...
    try:
        ...
        logger.debug("get_firebase_uid error: %s", e)
        return (None, None, None)
    ...
    try:
        ...
        with _lock:
            ...
            try:
                ...
                # Dedup within 24 h
                cur.execute(
                    "SELECT id FROM cases WHERE prompt_hash=? AND created_at>=? LIMIT 1",
            ...
            if parent_id is not None:
                cur.execute(
                    """SELECT id, prompt, risk_score, created_at, child_id
                       FROM cases
                       WHERE rlhf_pending=1 AND verdict='HESITATE' AND parent_id=?
